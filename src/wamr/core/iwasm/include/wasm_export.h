...
/**
 * Provide the value for an imported global so instantiation links it,
 * as if it had been resolved against a built-in module. Bytecode
 * modules only.
 *
 * @param module the WASM module
 * @param import_index index into the module's import section, which
 *        must refer to a global import
 * @param value the value to link the global to; the kind must match
 *        the declared type
 *
 * @return true on success
 */
WASM_RUNTIME_API_EXTERN bool
wasm_runtime_link_import_global(const wasm_module_t module,
                                int32_t import_index,
                                const wasm_val_t *value);

/**
 * Get the number of data segments in a WASM module. Bytecode modules
 * only; returns -1 otherwise.
 */
WASM_RUNTIME_API_EXTERN int32_t
wasm_runtime_get_data_segment_count(const wasm_module_t module);

/**
 * Get the linear memory range an active data segment initializes.
 * Passive segments report a zero-length range. Returns false for
 * segments whose offset is not a constant expression, or for
 * non-bytecode modules.
 */
WASM_RUNTIME_API_EXTERN bool
wasm_runtime_get_data_segment_range(const wasm_module_t module,
                                    int32_t index, uint64_t *out_offset,
                                    uint32_t *out_length);
